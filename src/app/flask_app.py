...
def _run_script_job(job_id, script_path, ip, port):
    ...
    # Prefer the same Python used by this process
    cmd = [sys.executable, script_path, "--ip", ip, "--port", str(port)]
    try:
        p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, env=env, cwd=SCRIPTS_DIR)
        out, _ = p.communicate()
...
def api_mode_status():
    ...
    return jsonify({"ok": True, "is_awake": is_awake, "animation_enabled": animation_enabled, "life_state": life_state})
